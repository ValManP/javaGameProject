...
public class GameCycle {
    ...
    public GameCycle(AirHockeyState m) {
        ...
        size = new Dimension(Physics.Field);
        ...
        mallets[0] = new Mallet(size.width / 2, Physics.MalletRadius + 10, Physics.MalletRadius);
    ...
    public AirHockeyState calculate(AirHockeyState oldState) {
        ...
        // Physics calculation
        process(oldState.getMallet1(), oldState.getMallet2());
    ...
    private synchronized Point getMallet1Position() {
        return state.getMallet1();
    }
    
    private synchronized Point getMallet2Position() {
        return state.getMallet2();
    }
    ...
    void process(Point m1, Point m2) {
        ...
        for (int i = 0; i < 2; ++i) {
            collision = puck.willCollide(mallets[i]);
            if (collision.willCollide) {
        ...
        if (min_k_wall < min_k_mallet) {  // Шайба сталкивается со стеной
            ...
            // Обработка гола
            if (wall_to_collide == walls[1]) {  // Гол забил 2 игрок
                handleGoal(true);
            }
            if (wall_to_collide == walls[5]) {  // Гол забил 1 игрок
                handleGoal(false);
            }
